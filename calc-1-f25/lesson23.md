# Calculus I Lesson 23: Finishing Up; Definite Integrals / Substitution
{:.no_toc}
...
* Function analysis: increasing, decreasing, concavity
* Antiderivatives
* FTC, Chain Rule

# Definite Integrals of Rates
...
In general: if $f(t)$ is the rate of change in some quantity at time $t$, then $\int_a^b f(t) dt$ means the total change from $a$ to $b$ of that quantity. Examples:

* Water leaking from a tank at a rate of $R(t)$ liters per hour, then $\int_a^b R(t) dt$ is the amount of water that leaks out between $t = a$ and $t = b$ hours.
* Population increasing at a rate of $f(t)$ people per year, then $\int_a^b f(t) dt$ is the total increase in the population between year $a$ and year $b$.
* The COVID-19 graphs example above: what's the rate, and what's the accumulated quantity?
