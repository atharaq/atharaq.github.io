...
$$f(x) = \begin{cases}kx^2 + 2 &\mbox{if } x < 1
\\ 4x - 1 &\mbox{if } x \geq 1 \end{cases}$$

A student is asked to find $k$ such that $f$ is differentiable at $x = 1$. The student does the following:

1. When $x < 1$, using the sum and constant multiple rules, $f^\prime(x) = 2kx$.
2. When $x \geq 1$, since $4x - 1$ is a line with slop e1, $f^\prime(x) = 4$.
3. Setting these equal to each other when $x = 1$ and solving, we get that $k = 2$.
...
<iframe src="https://www.youtube.com/embed/K6-AIFxqb-A" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"></iframe>
</div>
