...
<iframe src="https://www.youtube.com/embed/q_vbII-u0Jg" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"></iframe>
</div>
...
<div class="youtube-container">
<iframe src="https://www.youtube.com/embed/QlB6eKQYjvM" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"></iframe>
</div>
...
The `SpecificException` catch clause would never execute, because more specific exception would have been caught by the "general" exception class.
...
<div class="youtube-container">
<iframe src="https://www.youtube.com/embed/Am0zf_iqUGk" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"></iframe>
</div>

Often times, there is some code you wish to always run, regardless of whether an exception was thrown. Usually, this involves closing resources of some kind. For example, the following code (from [CopyFile.java](code/CopyFile.html)) is part of a program which makes a copy of a file. The code opens a file to read from it, creates a second file to write to, and copies things from the first one to the second.
...
It might appear like there is some magic here. But under the hood, this is using *polymorphism*, which we started looking at last week. The data type (class) of any "resource" which we declare in the parentheses next to "try" statement must implement the `AutoCloseable` interface (an interface which just has a `close` method which may throw an Exception). Recall: an interface is an abstract data type which defines the methods that an object can have. So any object of type `AutoCloseable` has a `close` method. 

The `InputStream` and `FileOutputStream` classes both implement that interface. We will revisit interfaces later this week, but basically this just means that both of these classeshave `close()` methods, and, importantly, the Java compiler knows that (since they implement the interface). After this `try-catch` block runs, the two close methods will be invoked, making sure to close each of these "resources" properly.

You can still use `finally` if needed for other purposes.

## Exercise

Download the [CopyFile class](code/CopyFile.html) and the [java.jpeg](code/java.jpeg) files. Create a new project (in IntelliJ or VSCode), and put the CopyFile class in there, and the java.jpeg file in the src folder as well. Modify the CopyFile class to use the try-with-resources paradigm. When you run the program, you should see a new file in the src folder: output.jpeg, which should look exactly the same as the original java.jpeg file.
...
<div class="youtube-container">
<iframe src="https://www.youtube.com/embed/2i99eepMNJk" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"></iframe>
</div>
...
We have seen something like this before. Remember the [error checking we did in the Time class](lesson8.html#error-checking):
...
You can `throw` any object whose type is "Exception". In other words, any class which **extends** from the Exception class.
...
<div class="youtube-container">
<iframe src="https://www.youtube.com/embed/k1HrkQEo8iU" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"></iframe>
</div>

We started talking about this already, but it's worth revisiting the idea of polymorphism in the context of exceptions. Recall that polymorhpism is the ability of one object to take on multiple forms. We are seeing this in several ways in this lesson:
